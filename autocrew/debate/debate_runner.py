...
Review the plan from YOUR role's perspective. Return JSON:
{{
...
Be honest and specific. Approve only if you have zero blockers and no major concerns.
...
def _format_other_critiques(critiques: list[AgentCritique], exclude_role: str) -> str:
    lines = []
    for c in critiques:
        if c.agent_role == exclude_role:
            continue
...
def _llm_critique(
    ...
    prompt = CRITIQUE_PROMPT.format(
        ...
        partial=partial,
        other_critiques=_format_other_critiques(prior_in_round, agent.role.value),
    )
...
def run_debate(
    ...
    for round_num in range(1, max_rounds + 1):
        ...
        for i, agent in enumerate(squad.agents, 1):
            if dual_router is not None:
                agent_llm, model_name = dual_router.for_agent(agent)
                short_model = model_name.split("/")[-1]
                _log_debate(
                    f"  [{i}/{len(squad.agents)}] {agent.name} ({short_model}) — waiting for API..."
                )
        ...
        if all_approved or (total_blockers == 0 and round_num >= 2):
